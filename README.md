...
- **The same question keeps coming back.** A same-day re-show (right after getting it wrong, or reviewing twice in one sitting) always reuses the cached text by design — that's a genuine "you just saw this" repeat, not a new review. Any later day writes a fresh phrasing automatically unless you've turned on Settings → "Reuse questions across days" to save on model calls.
